...
const flow = [
  { icon: <Mic size={18} />, label: 'Telugu Voice Note', sublabel: 'Audio message received', color: 'bg-coral-subtle text-coral' },
  { icon: <FileText size={18} />, label: 'Transcription', sublabel: 'Speech-to-text processing', color: 'bg-electric-blue-subtle text-electric-blue' },
  { icon: <Layers size={18} />, label: 'Structured Case', sublabel: 'AI-organized information', color: 'bg-teal-subtle text-teal' },
  { icon: <User size={18} />, label: 'Creator Review', sublabel: 'Human verification step', color: 'bg-green-subtle text-green' },
];
...
export default function VoiceFirstSection() {
  return (
    <section className="py-20 lg:py-28 bg-navy text-white relative overflow-hidden">
      {/* Background accent */}
      <div className="absolute top-0 right-0 w-96 h-96 bg-electric-blue/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 pointer-events-none" />

      <div className="max-w-7xl mx-auto px-5 sm:px-8 relative z-10">
        <AnimateOnScroll>
          <div className="max-w-3xl mx-auto text-center">
            <span className="inline-flex items-center gap-2 text-xs font-bold tracking-[0.2em] uppercase text-electric-blue-light mb-5">
              <Languages size={14} />
            ...
            <p className="mt-5 text-lg leading-relaxed text-slate-light max-w-2xl mx-auto">
              ...
              <React.Fragment key={step.label}>
                <div className="flex items-center gap-4 p-4 bg-white/5 border border-white/10 rounded-xl backdrop-blur-sm">
                  ...
                  <div>
                    ...
                    <span className="text-xs text-slate-light">{step.sublabel}</span>
                  </div>
                </div>
                {i < flow.length - 1 && (
        ...
        <AnimateOnScroll delay={400}>
          <div className="mt-10 text-center">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/10 rounded-full">
              <Globe size={14} className="text-electric-blue-light" />
